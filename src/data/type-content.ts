    ...
    termGroups: [
      {
        ...
        terms: [
          { zh: "? + tilt/smash/air", desc: "結合方向與攻擊類型的縮寫，F (前)、N (原地/空)、U (上)、D (下)、B (後)，搭配 Smash (猛擊)、Air (空中攻擊)、Tilt (強攻擊) 等組合使用（例如 F-Smash 為前猛擊、N-air 為空N）。" },
          { zh: "J-air", desc: "連招的縮寫，特指「Jab (弱攻擊) 快速轉向接 B-air (空後)」的連段。" },
          { zh: "T-bag", en: "Teabagging", desc: "連續蹲下的動作，帶有把自已放在對手臉上、嘲諷與羞辱對手的意味。" },
          { zh: "Tumble", en: "Reeling", desc: "角色被擊飛後，呈現類似布偶癱軟的無防備狀態即為 Tumble。若傷害超過100%時，有機率變為縱向旋轉擊飛的 Reeling 狀態。" },
          { zh: "Jab Lock", desc: "當對手倒地且未受身時，利用傷害較低的弱攻擊打中對手，使其繼續躺在地上（最多連續兩次），以利後續追擊。" },
          { zh: "Tech Chase", en: "受身追擊", desc: "預判並追擊對手受身位置的行為。" },
          { zh: "Trip", desc: "角色單純跌倒的狀態，例如被香蕉皮丟到而絆倒。" },
          { zh: "Crawl", desc: "將搖桿推向斜下方時，特定角色會做出的爬行動作。" },
          { zh: "Spike / Meteor", en: "隕石", desc: "具有向下高速擊飛判定的攻擊，只要超過特定條件就能提早將對手擊墜。" },
          { zh: "Kill Confirms", desc: "只要輸入時機和打中位置正確，就必定能夠接續擊墜對手的連招。" },
          { zh: "True Combo", en: "絕對連招", desc: "對手完全無法透過迴避脫離的必中連招。" },
          { zh: "Lab", desc: "在訓練模式中測試連招或遊戲機制的行為。" },
          { zh: "Footstool", en: "踩頭跳", desc: "在對手頭上按跳躍鍵，將對手當作踏板起跳，會造成對手短暫的無法動作時間。" },
          { zh: "Tomahawk", en: "空跳抓", desc: "跳起來後什麼都不做直接落地，藉由騙取對手舉盾防禦的時間差，在落地瞬間進行抓取。" },
          { zh: "Delay", en: "延遲", desc: "刻意晚一拍才出招，或是回場時改變慣性來製造落下位置的時間差。" },
          { zh: "Armor", en: "霸體", desc: "遭受攻擊時也不會停止當下動作的狀態，可細分為 Heavy、Passive、Super 等不同機制。" },
          { zh: "Flinch / Hitlag / Hitstun", desc: "被打中而後退的膽怯動作為 Flinch。雙方因攻擊接觸而多出的停滯時間稱為 Hitlag。被擊飛後無法動作的硬直時間則為 Hitstun。" },
          { zh: "Startup / Active Frames / Endlag", desc: "出招開始到產生判定所需的幀數（前硬直）為 Startup。動作過程中真正有攻擊判定的期間為 Active Frames。招式結束後的硬直時間則是 Endlag。" },
          { zh: "Hitbox / Hurtbox / Disjoint", desc: "攻擊判定範圍（Hitbox）與角色本身會受傷的範圍（Hurtbox）。若兩者不重合（如武器判定在身體之外），出招會相對安全，稱為 Disjoint。" },
          { zh: "Tipper / Reverse Tipper", desc: "劍尖部位具備強判定的招式（Tipper），以及劍柄部位具備強判定的招式（Reverse Tipper）。" },
          { zh: "Burst", desc: "出招速度極快、讓對手幾乎難以反應的攻擊。" },
          { zh: "Option / Option Coverage", desc: "在特定情形下能做出的選擇（Option）。利用一個動作去反制或涵蓋對手多種可能選項的行為稱為選項覆蓋（Option Coverage）。" },
      ...
      },
      {
        ...
        terms: [
          { zh: "Spam / Mash", desc: "不斷重複使用相同招式的行為稱為 Spam。不經思考、不觀察對手而無腦狂放猛擊的行為稱為 Mash。" },
          { zh: "Whiff Punish", en: "揮空懲罰", desc: "趁對手招式揮空之後，抓準破綻給予的懲罰攻擊。" },
          { zh: "Poke", desc: "朝對手可能接近的位置揮招，用於試探對手移動方向或限制其接近。" },
          { zh: "Clank / Trade", desc: "雙方攻擊判定互相抵消（Clank），或互換傷害（Trade）。" },
          { zh: "Staling", en: "Stale-move Negation", desc: "一直使用相同招式，會導致該招式的傷害量與安全幀數漸漸減少的機制。" },
          { zh: "Drag Down", en: "往下拖", desc: "故意不將招式打完，藉此把空中的對手拖至地面產生硬直，以利接續擊殺技。" },
          { zh: "Gimp", desc: "對處於場外的對手施加小傷害或干擾，使其錯過反應時間或操作失誤而遭擊墜。" },
          { zh: "Spacing / Zoning", desc: "隨時保持安全距離來攻擊對手（Spacing）。利用場地優勢或遠距離攻擊去限制對手行動、逼迫對方做出選擇（Zoning）。" },
          { zh: "Camp / Circle Camp", desc: "避免正面迎戰，傾向利用遠端攻擊打帶跑的風箏戰術（Camp）。在三平台場地環繞打帶跑的游擊戰則為 Circle Camp。" },
          { zh: "Juggle", desc: "持續將對手打飛在空中，並一直對空攻擊阻止對手輕易落地。" },
          { zh: "DI / LSI", en: "擊飛向量/速度調整", desc: "在被擊中時推動搖桿，藉此調整被擊飛的方向（DI）或影響擊飛速度的倍率（LSI）。" },
          { zh: "SDI", desc: "在被連續攻擊的硬直期間，重複輸入同向指令使角色漸漸移動，以提早脫離攻擊範圍。" },
          { zh: "Pressure", en: "施壓", desc: "對劣勢者施加壓力（例如壓盾），引誘或逼迫對手做出你預期的行動。" },
          { zh: "Condition", en: "制約/養成習慣", desc: "透過重複特定行為去影響對手，讓對手養成某種習慣後再趁機給予大懲罰。" },
          { zh: "Ledge Trap", en: "困邊", desc: "將對手困在邊緣，透過猜測或選項覆蓋讓對手難以回到場內。" },
          { zh: "Edgeguard", en: "守邊", desc: "主動阻止場外對手成功回場，甚至讓其連邊緣都抓不到的行為。" },
          { zh: "Mix Up", en: "混入選項/意外選項", desc: "在相同情境中混入不同的選擇避免被摸透，或是刻意使用出乎意料的選項來反制對手。" },
          { zh: "Call Out", desc: "成功猜中或看破對手的選項，並給予重擊懲罰。" },
          { zh: "Matchup / MU", desc: "針對角色之間對戰的優劣勢關係與好不好打的概念。" },
          { zh: "Game Plan", desc: "理解自身與對手角色特性後，所擬定的整體打法與戰術。" },
          { zh: "CQC", en: "Close Quarter Combat", desc: "可互相攻擊範圍內的近距離攻防戰。" },
          { zh: "Line", en: "Stage Control", desc: "雙方角色之間畫出的虛擬界線，代表角色的自由活動領域，領域越大代表場地控制權越穩固。" },
          { zh: "Advantage / Disadvantage", en: "優勢/劣勢", desc: "依據站位、資源或角色狀態，能做的事情較多的一方為優勢，反之為劣勢。" },
          { zh: "Neutral", en: "立回", desc: "雙方皆無優劣勢的局面，或是為了獲得優勢而互相試探、走位的進攻模式。" },
          { zh: "Ditto", desc: "使用相同角色進行的鏡像對戰。" },
          { zh: "Risk and Reward", en: "風險獎勵", desc: "評估操作失誤帶來的風險與成功帶來的獎勵之間的關係。" },
        ],
      },
    ],
    videos: [
