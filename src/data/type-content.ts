...
export const typeContent: Record<string, TypeContent> = {
  ...
  "totk-06": {
    ...
    principleSections: [
      {
        text: "「餘料建造（Fuse）」的作業流程玩家是全程__同步硬直__的，也就是「直到餘料建造完成的期間，玩家沒有任何操作可以中斷餘料建造（～Ver.1.1.2版有__冷切盾__，就是強制打開快速選單切換裝備來中止餘料的動作）。而「餘料儲存（Fuse Storage）」能讓玩家先__向系統發起餘料建造的要求__，但是不執行餘料建造，只是先儲存了「餘料要求的申請」，之後玩家就可以再重新觸發餘料建造的時候，自由控制要在哪個時間點中斷餘料，進而產生「餘料糾纏(FE)」「冷餘料(CF)」等行為。",
      },
      {
        title: "Frame 0",
        collapsible: true,
        text: "對準目標輸入餘料建造的指令（如按 ZL 或 Y）。林克會向系統發出「執行餘料建造的申請」，並且檢查要餘料的目標物品是否非死亡(Dead)、隱藏(Paused)、附著(Attached)，「餘料儲存（Fuse Storage）」會在此幀中斷。",
      },
      {
        title: "Frame 1",
        collapsible: true,
        text: "開始餘料建造的第 1 幀。要餘料的目標物品會向系統發出「建立依賴的申請（子 → 母）」，依賴關係的對象會設定在「物品要附著的裝備（Parent）」上，並將物品移動到林克的裝備上準備執行附著（Attaching）。若於此幀執行「丟棄→再裝備並卸掉」，由於系統找不到「附著對象（Owner）」，因此只會看到物品飛到被丟棄裝備的位置、保留最初的依賴關係，但是選單上和視覺上都不會有附著在裝備的外觀（即「無特效冷餘料（Non-Animated Cold Fuse）」）。此幀也可以利用快速進出隱藏區的方式，讓程序往前推 1 幀，在 Frame 1 的狀態儲存下來（即「第二類餘料儲存（FS2）」），此狀態已經進入餘料和建立依賴的程序，只差還沒附著，因此更好靠操作中斷餘料建造的流程。",
      },
      {
        title: "Frame 2",
        collapsible: true,
        text: "開始餘料建造的第 2 幀。觸發餘料特效，並且將要餘料的目標物品正式附著在依賴對象（要附著的裝備）上，物品的狀態更新為「已附著（Attached）」，並完成「模型結合（Model Bind）」。若於此幀執行「丟棄→再裝備並卸掉」（FS2 則是丟棄），「附著對象（Owner）」會失效，因此會看到有餘料特效、物品飛到被丟棄裝備的位置、保留最初的依賴關係，但是選單上和視覺上都不會有附著在裝備的外觀（即「有特效冷餘料（Animated Cold Fuse）」）。若於此幀執行「切換裝備」或「卸掉再重新裝備」（FS2 則是切換），改變了「附著對象（Owner）」，因此會看到有餘料特效、物品附著到被切換的裝備上。",
      },
      {
        title: "Frame 3",
        collapsible: true,
        text: "開始餘料建造的第 3 幀。依賴對象更新狀態為「有附著東西」並「建立依賴的申請（母 → 子）」，目標物品會檢查「當初的附著對象（Owner）」和「目前附著的裝備（Parent）」是否相同。「餘料糾纏(FE)」的子物件會分離就是在此幀上，裝備認為它自己是 Parent、有附著物品，但物品發現 Owner 無效、自己不屬於目前的裝備而「分離（Detached）」，但是保留依賴關係。「假餘料(PF)」通常發生在冷餘料一個糾纏在其他裝備上的物品。因此假餘料和餘料糾纏相同，只是物品還保留著 Owner 沒有被消滅（餘料糾纏的母物件還在），因此在這個流程上不會「分離（Detached）」。",
      },
      {
        title: "Frame 4",
        collapsible: true,
        text: "開始餘料建造的第 4 幀。解析依賴關係器 BaseProcMgr 分析 Frame 3 送出的申請，穩定依賴關係。",
      },
    ],
    notes: [
    ...
    methods: [
      {
        ...
        tags: ["～Ver.1.1.1"],
        steps: [
          "讓林克處於空中狀態（shield jumping、paragliding，或站在米涅魯上）",
          "打開暫停選單並持有（hold）道具",
          "按 Y 排序選單，同時關閉選單",
          "撿起道具",
        ],
        note: [
          ...
          "Y 鍵與「退出選單」必須在同一個 frame 按下。",
        ],
        principle: "在空中複製材料：於 midair 狀態下暫停並持有道具，同一 frame 內排序選單並關閉選單即可複製。",
      },
      {
        tab: "ZSD",
        tags: ["～Ver.1.1.1"],
        ...
        note: [
          "若遊戲出現 lag，代表這次會成功；如果沒有 lag，代表站得太近了。如果道具數量減少、不變，或跟你取出的數量對不上，就代表失敗，或是部分左納烏裝置已經生成到世界中了。",
          "想要更穩定的做法：離牆面約 3 個 shield parries 的距離，只要牆面夠寬通常有效；如果遊戲沒有 lag，試著離遠一點，但林克仍要面向牆壁；轉角牆面或密閉空間（例如濱海科技實驗室）效果最好。",
        ],
        principle: "把左納烏零件複製回背包，做法與 MSD 相近，但需面向牆壁或懸崖以觸發遊戲的 lag 判定。",
      },
      {
        tab: "MTD",
        tags: ["All Versions"],
      ...
      },
      {
        ...
        steps: [
          ...
          "產生器的艙門應保持開啟，若沒有請重試",
          "取出一個 stabilizer 並啟動它",
          "用究極手把 stabilizer 放進艙口，卡住艙門讓它保持無限期開啟",
          "把想要的道具全部放進產生器，這些存放的道具數量可以超過 21 個上限",
          "想收取時，把 stabilizer 從艙口移開，讓道具全部掉出來。要盡快撿，超過 21 個上限的部分會開始 despawn",
        ],
        note: [
          "可搭配 SID 或 MTD 一起使用，以達到道具數量上限。",
          "在艙口旁放一個 Fan 或 Stabilizer，可以讓艙門下次打開後不會關閉。",
          "用兩個以 45 度角餘料在一起的 Fan 可以卡住產生器，避免 Stabilizer 有效時間限制的問題（頂部的 Fan 也可換成一顆小石頭）。",
        ],
      ...
      },
      {
        ...
        sections: [
          {
            title: "Method 1（發現者：鯉神 Li Shen）",
            steps: [
              "Unsheathe（拔出）一把弓",
              "Fuse 一個道具到箭矢上",
              "Pause",
              "Drop 你裝備中的弓，並 swap 到另一把弓",
              "Pause buffer（盡快 unpause 再 pause）",
              "Drop 你新裝備的弓",
              "Unpause",
              "從步驟 1 重複",
            ],
          ...
          {
            title: "Method 3（發現者：mulberry）",
            steps: [
              "對一把或多把弓做 Drop Zuggle，或 Detached Drop Zuggle",
              "Equip 另一把弓",
              "Fuse 一個道具到箭矢上",
              "Drop 你裝備中的弓（不會影響你的 drop zuggles）",
              "從步驟 2 重複",
            ],
          },
        ],
        ...
        principle: "可複製任何能餘料到箭矢上的材料，包括左納烏裝置。此 glitch 依賴弓的處理順序（processing order），順序可以一定程度上被操控，但永遠帶有隨機性，載入的物件越少，成功率越穩定。",
      },
      {
        tab: "SID",
        tags: ["Ver.1.2.0+"],
        steps: [
          "對目標素材做 Stack Splitting，拆成 1 個一堆、以及另一堆 2 個(以上)",
          "進入任意 midair 狀態（shield surf、paraglide、Mineru 等），然後 pause",
          "從第二堆（拆出來、數量較多的那堆）拿起你能拿的最大數量",
          "sort 選單，或是打開地圖",
          "unpause",
        ],
        principle: "透過 Stack Splitting 複製材料：把同種素材拆成數量不同的兩堆，在 midair 狀態下暫停時拿取較大堆的最大數量，再排序選單或開地圖後解除暫停即可複製。",
      },
      {
        tab: "ZISD",
        tags: ["All Versions"],
        steps: [
          ...
          "不要離開選單，切到地圖畫面（按 −），並 watch a memory（觀看一段回憶）",
          "切回 Zonai Devices 分頁（按 +），然後退出選單",
        ],
        principle: "利用 Memory Buffering 和 MTD 複製左納烏裝置。需求：要複製的裝置至少 1 個，另一個裝置要有 10 個，且在選單中排序要緊接在「想複製的裝置」之後（即在暫停選單中位於其右側）。",
      },
      {
        tab: "BID",
        tags: ["All Versions"],
        steps: [
          "設置 sdc culling 或類似的 stick culling（部分地點比其他地點更穩定；like-like 出沒的 look-out landing 這類地點效果很差）",
          "在林克 cull 前 1 frame 持有一個道具（若這個時機點抓不準，可改為：watch a memory 來 uncull，在回憶播放期間按住 L 且不放開，等 1 秒，再 unpause 並選擇地圖 rune，需要抓到特定的 cull luck）",
          "用某種方式 unload 該道具，其中一種方法是讓馬吃掉它（只對蘋果有效）",
          "把 unload 後的道具丟到地上",
          "騎上馬，讓 culling 停止",
          "下馬，並持有 5 個剛才 unload 的道具",
          "Unpause",
          "放開持有其中 4 個道具，改成持有你想複製的道具",
          "準備丟出一個道具，這樣才能撿東西",
          "要複製時：先取消持有一個複製出來的道具，再重新持有它，然後 unpause",
          "撿起道具，並重複步驟 9～11",
        ],
        principle: "用特定方式 culling drop 一個道具並讓它 unload，會讓它進入「持有其他道具時不斷 respawn」的狀態，可以大量複製。",
      },
    ],
    notes: [
