...
export const typeContent: Record<string, TypeContent> = {
  ...
  "botw-01-ragdoll": {
    videoFolder: "botw-launch",
    // 示意影片待補：檔案放進 media-source/type-videos/botw-launch/ 之後，
    // 把下面兩個 method 的 video 欄位填上檔名，並移除 showEmptyMedia
    showEmptyMedia: true,
    // 這頁講的不是「怎麼做一種擊飛」，而是「怎麼中招、怎麼解除」，
    // 沿用預設的「流程步驟」會對不上內容
    methodsTitle: "觸發與解除",
    methods: [
      {
        tab: "怎麼中招",
        name: "布偶程錯（Ragdoll Glitch）",
        tags: ["All Versions"],
        intro: "這個程錯絕大多數是不小心觸發的——只要做過下面這個動作，接下來的擊飛就可能全部失效：",
        steps: [
          "踩盾跳（Shield Jump）",
          "在動作動畫還沒結束前，從選單__卸下盾牌__",
        ],
        note: [
          "※ 觸發的瞬間沒有任何提示，畫面上完全看不出異狀。",
          "※ 以「卸下盾牌的那個位置」為原點，四周會留下一塊仍然正常的範圍，走出去才會發作（見右側「安全範圍」）。",
        ],
      },
      {
        tab: "怎麼解除",
        name: "布偶修復（Ragdoll Fix）",
        tags: ["All Versions"],
        intro: "解除的動作跟觸發的動作一模一樣，差別只在於你是刻意做的、而且是在對的地點做：",
        steps: [
          "先移動到接下來__真正要擊飛的位置__",
          "在該位置踩盾跳（Shield Jump）",
          "在動作動畫還沒結束前，從選單卸下盾牌",
        ],
        note: "※ 這麼做會把安全範圍的原點刷新到目前位置，原本因為走太遠而失效的擊飛就會恢復正常。",
      },
    ],
    principleSections: [
      {
        text: "[[風彈|botw-01]]、[[一般擊飛|botw-01-stasis]]、[[擊飛加速|botw-01-super]] 都靠同一件事運作：讓林克進入__布偶狀態（Ragdoll）__——失去控制、隨外力被擊飛翻滾的物理狀態。「布偶程錯」就是讓林克__進不去__這個狀態的程錯；一旦中招，炸彈、靜止器撞擊、爆炸傷害都推不動他，擊飛直接失敗。",
      },
      {
      ...
      },
      {
        title: "為什麼走遠了就會失敗",
        collapsible: true,
        text: "超出範圍不是「效果被直接關掉」，而是遊戲在計算布偶物理時，會額外給林克一份隱藏的__自轉力矩（Spin）__，而且__距離原點越遠，這份力矩就倍數暴增__。過強的自轉會干擾正常的受力擊飛，讓林克變成原地打轉，而不是被推出去。",
      },
      {
        title: "把看不見的自轉「看」出來",
        collapsible: true,
        text: "這份力矩平常是看不見的。利用電擊陷阱讓林克觸電後落進__淺水__，就能直接觀察到它——林克會以極高速度原地瘋狂自轉，正是這股異常自轉在干擾擊飛。",
      },
      {
        title: "朝向的影響與實戰應用",
        collapsible: true,
